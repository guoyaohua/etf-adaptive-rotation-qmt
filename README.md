...
```math
Score_i = M_i / max(σ_{40,i}, 0.10)
```

其中 $R_{n,i}^{(-5)}$ 是跳过最近 5 日后的 $n$ 日收益，$σ_{40,i}$ 是 40 日年化波动率。分数必须为正。
...
| 风控层 | 默认规则 | 作用 |
|---|---:|---|
| 初始止损 | 入场价减 2.5 × ATR(20) | 限制单资产初始风险 |
| 跟踪激活 | 浮盈达到 1.5 × ATR | 避免过早收紧止损 |
| 跟踪止损 | 峰值减 3.0 × ATR | 让利润奔跑并控制回吐 |
