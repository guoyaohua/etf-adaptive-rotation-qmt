...
2. **优先买谁**：比较跳过最近 5 日的 20/60/120 日加权动量，再用 40 日年化波动率惩罚高波动资产。
3. **买多少**：最多选择 3 个不同风险组、相关性不过高的 ETF，先做逆波动率配置，再缩放到 10% 目标波动率，并保留至少 10% 现金。
4. **何时更新**：每周五收盘形成一份新子组合，替换四周前的子组合；新目标最早在下一交易日执行。

因此，完整组合是最近四个周五信号的平均，而不是每四周一次性全仓换股。趋势不足时允许持有更多现金，甚至全部现金。
...
| 层级 | 默认周期 | 实际含义 |
|---|---|---|
| 信号计算 | 每个有行情的周五收盘后 | 生成一份全新的候选组合 |
| 单个子组合 | 约四周 | 本周新信号替换四周前信号 |
| 完整组合 | 每周滚动更新 | 每次通常只更换约四分之一的信号来源 |
| 回测执行 | 下一交易日开盘 | 周五信号不会用周五收盘价成交 |
...
节假日需要额外注意：当前调度器只认实际行情日中的星期五。如果星期五休市，该周不会自动改到星期四生成信号；下一次例行信号是下一个有行情的星期五。

## 第一层：标的池与数据资格

默认池包含跨境股票、黄金与国债 ETF。每个品种配置了：

- `role`：`growth`（股票风险资产）或 `defensive`（黄金、国债）；
...
10% 波动率地板防止极低的短期波动把分数异常放大。这个分数只用于候选排序，不直接等于目标仓位，也不代表预期收益。
...
```math
Stop_{\mathrm{initial}} = E - 2.5 \times ATR_{20}
```

当持仓最高价相对入场价至少上涨 $1.5 \times ATR_{20}$ 后，启用跟踪候选：

```math
Stop_{\mathrm{trailing}} = HighWatermark - 3.0 \times ATR_{20}
```

实际止损价取初始止损和跟踪候选中较高者。ATR 固定为入场时数值，不随持仓期间重新计算。若开盘已经低于止损价，回测按更差的开盘价退出；否则日内最低价触及止损时按止损价退出。
...
- `live-once` / `scripts/live.ps1`：刷新已完成日线、生成周度目标、对账账本，并生成或提交一次再平衡批次；
...
sequenceDiagram
    ...
    O->>R: 执行前一日触发的组合清仓
    O->>P: 执行上一决策日留下的目标（先卖后买）
    B->>R: 用当日 OHLC 检查单资产止损
    B->>R: 收盘更新净值、回撤与冷却状态
    B->>S: 若为周五，用截至今日收盘的数据生成信号
    S-->>P: 保存为下一交易日才可执行的目标
```

特别是，周五收盘后才知道的信号绝不会按同一个周五的收盘价成交。
...
| `selected.*.score` | 动量除以波动率地板后的排序分数 |
...
| 仓位 | 单资产 / 总仓位上限 | 40% / 90% |
| 交易 | 最小权重变化 | 1% |
| 止损 | 初始 / 激活 / 跟踪 ATR 倍数 | 2.5 / 1.5 / 3.0 |
