...
- 新目标最早在下一交易日执行，不使用周五收盘后才知道的信息在周五成交；
- 周五休市时，当前实现不会提前到周四，而是跳过该周例行信号。
...
4. **风险调整排名**：用 40 日年化波动率惩罚高波动资产，分数必须为正。
5. **去除重复暴露**：每份信号最多 3 只 ETF；同一风险组最多一只，与已选资产的 60 日相关系数不得高于 0.85。
6. **仓位计算**：先做逆波动率分配，再缩放至 10% 组合目标波动率；单资产不高于 40%，总风险仓位不高于 90%，不使用杠杆。
7. **四份信号聚合**：最近四个有效周五的目标各占完整组合的 25%，不足四份的部分保留现金。
8. **订单差额**：按策略资金上限和实时价格换算整手数量，先卖后买；目标变化不足策略资金的 1% 时不交易。
...
```math
Score_i = \frac{M_i}{\max(\sigma_{40,i}, 0.10)}
```
...
这里 $R_{n,i}^{(-5)}$ 是跳过最近 5 日后的 $n$ 日收益，$\sigma_{40,i}$ 是 40 日年化波动率。完整公式、边界条件、调度示例和输出字段解释见 [策略完整说明](docs/STRATEGY.md)。
...
| 层级 | 默认值 | 含义 |
|---|---:|---|
| 初始止损 | 入场价 − 2.5 × ATR(20) | 限制单资产初始风险 |
| 跟踪激活 | 浮盈达到 1.5 × 入场 ATR | 避免过早收紧 |
| 跟踪止损 | 最高价 − 3.0 × 入场 ATR | 控制盈利回吐 |
...
`configs/local.yaml` 通过 `extends: strategy.yaml` 继承默认参数，只需保留本机覆盖项。它和 `.env`、行情、报告、运行账本都不得提交 Git。账号、密码、Key、Token 和真实 QMT 路径不要写入 YAML 或脚本；本项目只从 `QMT_CLIENT_PATH`、`QMT_ACCOUNT_ID` 环境变量读取账户相关信息。环境变量默认只在当前 PowerShell 会话有效。
...
它会刷新已完成日线、聚合最近四份周五信号、对账成交、检查账户与策略持仓是否混用，并把计划写入 `runtime/latest_order_plan.json`。默认只展示计划，**不会下单**。
