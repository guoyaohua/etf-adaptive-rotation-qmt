...
3. **买多少**：最多选择 3 个不同风险组、相关性不过高的 ETF，先做逆波动率配置，再缩放到 10% 目标波动率；剩余预算仅在国债趋势与动量为正时使用最多 30% 的国债现金代理。
...
初始权重和波动率缩放后若仍有闲置预算，策略可选择 `treasury` 组中长期趋势向上、加权动量为正且数据合格的最高分 ETF，补充至最多 30%。该代理不要求短期 EMA 斜率为正，以减少低波动资产的短周期噪声；但长期趋势或绝对动量为负时完全不启用。加入后再次检查组合波动率、单资产 40% 与总仓位 90% 上限。
